...
pub struct MarkedNode<'a, K: 'a, V: 'a, B: 'a> {
    node: &'a Node<K, V, B>,
    seen_l: bool,
    seen_r: bool,
}

impl<'a, K, V, B> Clone for MarkedNode<'a, K, V, B> {
    fn clone(&self) -> Self { *self }
}

impl<'a, K, V, B> Copy for MarkedNode<'a, K, V, B> {}

impl<'a, K, V, B> MarkedNode<'a, K, V, B> {
    pub fn new(node: &'a Box<Node<K, V, B>>) -> Self {
        MarkedNode { node: node, seen_l: false, seen_r: false }
    }
}

impl<'a, K, V, B> NodeRef for MarkedNode<'a, K, V, B> {
    type Key = K;
    type Item = (&'a K, &'a V);
...
pub struct MutMarkedNode<'a, K: 'a, V: 'a, B: 'a> {
    node: *mut Node<K, V, B>,
    seen_l: bool,
    seen_r: bool,
    _marker: PhantomData<&'a mut Node<K, V, B>>,
}

impl<'a, K, V, B> MutMarkedNode<'a, K, V, B> {
    pub fn new(node: &'a mut Box<Node<K, V, B>>) -> Self {
        MutMarkedNode { node: &mut **node, seen_l: false, seen_r: false, _marker: PhantomData }
    }
}

unsafe impl<'a, K, V, B> Send for MutMarkedNode<'a, K, V, B> where K: Send, V: Send, B: Send {}
unsafe impl<'a, K, V, B> Sync for MutMarkedNode<'a, K, V, B> where K: Sync, V: Sync, B: Sync {}

impl<'a, K, V, B> NodeRef for MutMarkedNode<'a, K, V, B> {
    type Key = K;
...
impl<K, V, B> NodeRef for Box<Node<K, V, B>> {
    type Key = K;
    type Item = (K, V);
